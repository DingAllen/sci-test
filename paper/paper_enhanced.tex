...
\textbf{Experimental Setup}: We trained a convolutional neural network on the real CIFAR-10 dataset (50,000 training images, 10,000 test images) using standard data normalization. The network consists of three convolutional blocks with batch normalization followed by fully connected layers. We trained for 30 epochs with batch size 256, using standard hyperparameters for each optimizer.

\textbf{Training Protocol}: Each optimizer used SGD variants with momentum 0.9 and weight decay 5e-4 (learning rate 0.01), or Adam with learning rate 0.001. SAM variants used perturbation radius $\rho=0.05$. Sharpness was measured using neighborhood perturbation analysis on the test set.
